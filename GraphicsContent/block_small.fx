...
static float3 edge_color = float3(1,0,0);
...
texture tex_mask;
sampler2D ss_mask = sampler_state {
	texture = <tex_mask>;
	addressU = Clamp;
	addressV = Clamp;
	mipfilter = LINEAR;
...
float4 ps_block(PS_IN input) : COLOR0 {
	...
	//
	float e = 0;
	float4 mask = tex2D(ss_mask, input.Tex);
	e += mask.r * input.Mask.r;
	e += mask.g * input.Mask.g;
	e += mask.b * input.Mask.b;
	e += (1-mask.a) * input.Mask.a;
	e *= saturate(input.Nor.y);
	col.rgb = lerp(col.rgb, edge_color, saturate(e));
	//
